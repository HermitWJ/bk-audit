...
2. 通用字段：raw_name 必须来自字段上下文，keys 为 []；拓展字段（下钻）：raw_name 取字段上下文中的 JSON 容器字段（如 extend_data）
   keys 为下钻子键——字段上下文已列出的照抄，未列出但用户明确指定的按用户描述的子键名生成
...
7. 关键词全文检索用 log 字段的 match_all/match_any 操作符表达：多个关键词需同时满足用 match_all，任一满足用 match_any
   当用户以中文或口语描述操作类型、资源类型等，而字段上下文的 options 与 sample_value 均无法确定该字段确切取值时
   禁止猜测字段值，改用 log 的 match_any 表达该关键词需求
